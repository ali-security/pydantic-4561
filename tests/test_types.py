...
def test_constrained_list_constraints():
    ...
    assert exc_info.value.errors() == [
        {"loc": ("v",), "msg": "value is not a valid list", "type": "type_error.list"}
    ]
...
def test_conlist():
    ...
    with pytest.raises(ValidationError) as exc_info:
        Model(foo=1)
    assert exc_info.value.errors() == [
        {"loc": ("foo",), "msg": "value is not a valid list", "type": "type_error.list"}
    ]
...
def test_constrained_set_constraints():
    ...
    with pytest.raises(ValidationError) as exc_info:
        ConSetModelBoth(v=1)
    assert exc_info.value.errors() == [
        {"loc": ("v",), "msg": "value is not a valid set", "type": "type_error.set"}
    ]
...
def test_conset():
    ...
    assert Model(foo=[1, 1, 1, 2, 2], bar=["spoon"]).dict() == {
        "foo": {1, 2},
        "bar": {"spoon"},
    }
    ...
    with pytest.raises(ValidationError) as exc_info:
        Model(foo=1)
    assert exc_info.value.errors() == [
        {"loc": ("foo",), "msg": "value is not a valid set", "type": "type_error.set"}
    ]
    ...
    assert exc_info.value.errors() == [
        {
            ...
            "type": "type_error.pyobject",
            "ctx": {
                "error_message": 'Module "os" does not define a "missing" attribute'
            },
        }
    ]
...
def test_tuple_fails(value):
    ...
    with pytest.raises(ValidationError) as exc_info:
        Model(v=value)
    assert exc_info.value.errors() == [
        {"loc": ("v",), "msg": "value is not a valid tuple", "type": "type_error.tuple"}
    ]
...
def test_uuid_error():
    ...
    assert exc_info.value.errors() == [
        {"loc": ("v",), "msg": "value is not a valid uuid", "type": "type_error.uuid"}
    ]
...
@pytest.mark.parametrize(
    "value,result",
    (("/test/path", Path("/test/path")), (Path("/test/path"), Path("/test/path"))),
)
...
def test_path_validation_fails():
    ...
    with pytest.raises(ValidationError) as exc_info:
        Model(foo=123)
    assert exc_info.value.errors() == [
        {"loc": ("foo",), "msg": "value is not a valid path", "type": "type_error.path"}
    ]
...
@pytest.mark.skipif(
    sys.platform.startswith("win"), reason="paths look different on windows"
)
@pytest.mark.parametrize(
    "value,errors",
    (
...
def test_new_type_success():
    ...
    assert (
        repr(Model.__fields__["c"])
        == "ModelField(name='c', type=List[int], required=True)"
    )
...
def test_json_optional_simple():
    ...
    assert JsonOptionalModel(json_obj='["x", "y", "z"]').dict() == {
        "json_obj": ["x", "y", "z"]
    }
...
def test_json_explicitly_required():
    ...
    assert JsonRequired(json_obj='["x", "y", "z"]').dict() == {
        "json_obj": ["x", "y", "z"]
    }
    with pytest.raises(ValidationError) as exc_info:
        JsonRequired()
    assert exc_info.value.errors() == [
        {"loc": ("json_obj",), "msg": "field required", "type": "value_error.missing"}
    ]
...
def test_pattern():
    ...
    assert Foobar.schema() == {
        ...
        "title": "Foobar",
        "properties": {
            "pattern": {"type": "string", "format": "regex", "title": "Pattern"}
        },
        "required": ["pattern"],
    }
    ...
    assert exc_info.value.errors() == [
        {
            ...
            "type": "value_error.regex_pattern",
        }
    ]
...
def test_secretstr():
    ...
    with pytest.warns(
        DeprecationWarning, match=r"`secret_str.display\(\)` is deprecated"
    ):
...
def test_secretstr_error():
    ...
    assert exc_info.value.errors() == [
        {"loc": ("password",), "msg": "str type expected", "type": "type_error.str"}
    ]
...
def test_secretbytes():
    ...
    with pytest.warns(
        DeprecationWarning, match=r"`secret_bytes.display\(\)` is deprecated"
    ):
        assert f.password.display() == "**********"
    with pytest.warns(
        DeprecationWarning, match=r"`secret_bytes.display\(\)` is deprecated"
...
def test_secretbytes_error():
    ...
    with pytest.raises(ValidationError) as exc_info:
        Foobar(password=[6, 23, "abc"])
    assert exc_info.value.errors() == [
        {"loc": ("password",), "msg": "byte type expected", "type": "type_error.bytes"}
    ]
...
def test_generic_without_params():
    ...
    m = Model(
        generic_list=[0, "a"], generic_dict={0: "a", "a": 0}, generic_tuple=(1, "q")
    )
    assert m.dict() == {
        "generic_list": [0, "a"],
        "generic_dict": {0: "a", "a": 0},
        "generic_tuple": (1, "q"),
    }
...
def test_default_union_types():
    ...
    assert DefaultModel.schema() == {
        ...
        "type": "object",
        "properties": {
            "v": {"title": "V", "anyOf": [{"type": t} for t in allowed_json_types]}
        },
        "required": ["v"],
    }
...
def test_smart_union_compound_types():
    ...
    assert Model(values=("L1",)).dict() == {
        "values": {"L": "1"}
    }  # expected coercion into first dict if not a list
